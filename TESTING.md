...
- **2台目のPCから接続する方法**（アカウント1つ・PC2台の場合）:
  ...
  **2台目（PC2）の準備:**
  ...
  > **注意**: 同じMicrosoftアカウントで2台接続しようとすると「あなたは別の場所で既にログインしています」と弾かれます。PC2では **別のプレイヤー名** を使う必要があります。

  **方法A: 別のMicrosoftアカウントを使う（確実）**
  1. PC2で別のMicrosoftアカウントでMinecraft Java版を起動

  **方法B: Prism Launcher（無料）でオフラインプロファイルを使う**
  1. [Prism Launcher](https://prismlauncher.org/) をPC2にインストール
  2. 起動後、右上の「アカウントを追加」→「**オフライン**」を選択
  3. プレイヤー名に `Player2` など（PC1と違う名前）を入力
  4. 1.21.1のインスタンスを作成して起動
  5. 「マルチプレイ」→「サーバーを追加」
  6. サーバーアドレスに **PC1のIPアドレス**（例: `192.168.1.10`）を入力
  7. 接続（`online-mode=false` なのでオフラインプロファイルでも接続可能）
